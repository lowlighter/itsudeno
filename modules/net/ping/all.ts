...
Module.register(
  ...
  class extends Module {
    ...
    async apply({args: {host, packets}}: before) {
      //Ping target host
      const {success, stdout, stderr} = await run(`ping -c ${packets} ${host}`)
      const result = {host, ip: null, transmitted: NaN, received: NaN, loss: NaN} as result

      //Parse result
      if (success) {
        const {transmitted, received, loss} = stdout.match(/^(?<transmitted>\d+).+?transmitted, (?<received>\d+).+?received, (?<loss>\d+).+?loss/m)?.groups ?? {}
        result.ip = stdout.match(/PING .* \((?<ip>[.\da-f:]+)\)/)?.groups?.ip ?? null
