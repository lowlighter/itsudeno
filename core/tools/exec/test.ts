// Imports
import { assert, assertMatch, assertObjectMatch, assertRejects, assertStrictEquals, Docker, ItsudenoError, Suite, TestTracer } from "../../testing/mod.ts"
import { exec, pwsh, sh } from "./mod.ts"
...
await new Suite(import.meta.url).group("exec", test => {
	...
	test("(?) throws ItsudenoError", async () => {
		const tracer = Object.assign(await new TestTracer().ready, {
			handle() {
	...
	test("() [edge cases]", async () => {
		...
		assertObjectMatch(
			await exec("bash -i", {tracer, prompts: {list: [{prompt: false, stdin: "echo itsudeno\n", lf: false}, {prompt: false, stdin: "exit", capture: false, amend: true}]}}),
			{success: true},
		)
	...
	test("()", async () => {
		const tracer = await new TestTracer().ready
		const {success, code, commands} = await sh(["echo itsudeno >&1", "echo itsudeno >&2", "echo itsudeno >&1", "echo itsudeno >&2"], {tracer})
		assert(success)
		assertStrictEquals(code, 0)
	...
	test("()", async () => {
		const tracer = await new TestTracer().ready
		const {success, code, commands} = await pwsh(["Write-Output itsudeno", "Write-Error itsudeno", "Write-Output itsudeno", "Write-Error itsudeno"], {tracer})
		assert(success)
		assertStrictEquals(code, 0)
		assertStrictEquals(commands.length, 4)
		assertObjectMatch(commands[0], {stdout: "itsudeno\n", stderr: ""})
