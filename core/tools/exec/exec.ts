...
import type { _prompt, command, cursor, exec, listener } from "./types.ts"
...
export async function exec(
	command: string,
	{tracer = null, prompts = {list: []}, piped = true, bounce = settings.tools.exec.bounce, poll = settings.tools.exec.polling, cwd, env, ansi = false}: exec = {},
) {
...
async function handle(process: Deno.Process, {tracer, channel, stdio, closed, prompts}: listener) {
	if (process.stdin) {
		if (prompts.list.length) {
			...
			if (
				((!prompt.stdout) && (!prompt.stderr) && (!prompt.prompt)) || (prompt?.[channel]?.test?.(stdio[channel]))
				|| ((prompt.prompt) && (prompts.channel === channel) && (prompts.string?.test(stdio[channel])))
			) {
				...
				const {stdin, stdout, stderr, lf = true, capture = true, amend = false, clean = true, flush = false, close = false, feedback = true} = prompts.list
					.shift()! as _prompt
